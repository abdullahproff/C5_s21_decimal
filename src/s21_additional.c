...
void converting_decimal_to_longdec(s21_decimal d, long_dec *result) {
...
int mantissa_is_greater_long(long_dec number_1, long_dec number_2) {
  int flag = 0;
  for (int i = 191; i >= 0; i--)
    if (get_bit_long(number_1, i) != get_bit_long(number_2, i)) {
      flag = get_bit_long(number_1, i);
      break;
    }
  return flag;
}

int mantissa_is_less(long_dec number_1, long_dec number_2) {
  return mantissa_is_greater_long(number_2, number_1);
}

int mantissa_is_equal(long_dec number_1, long_dec number_2) {
  int flag = 1;
  for (int i = 0; i < 192; i++) {
    if (get_bit_long(number_1, i) != get_bit_long(number_2, i)) {
      flag = 0;
...
int mantissa_is_less_or_equal(long_dec number_1, long_dec number_2) {
  return mantissa_is_less(number_1, number_2) ||
         mantissa_is_equal(number_1, number_2);
}

int mantissa_is_not_equal(long_dec number_1, long_dec number_2) {
  return !mantissa_is_equal(number_1, number_2);
}

int mantissa_is_greater_or_equal(long_dec number_1, long_dec number_2) {
  return mantissa_is_greater_long(number_1, number_2) ||
         mantissa_is_equal(number_1, number_2);
}
...
void add(long_dec number_1, long_dec number_2, long_dec *result) {
  if (get_sign_long(number_1) != get_sign_long(number_2)) {
    set_sign_long(&number_2, get_sign_long(number_1));
    return sub(number_1, number_2, result);
  }

  long_dec sum = {0};
  set_sign_long(&sum, get_sign_long(number_1));
  set_exp_long(&sum, get_exp_long(number_1));

  int temp = 0;
  for (int i = 0; i < 192; i++) {
    int b1 = get_bit_long(number_1, i);
    int b2 = get_bit_long(number_2, i);
...
void sub(long_dec number_1, long_dec number_2, long_dec *result) {
  if (get_sign_long(number_1) != get_sign_long(number_2)) {
    set_sign_long(&number_2, get_sign_long(number_1));
    return add(number_1, number_2, result);
  }

  long_dec diff = {0};
  set_sign_long(&diff, get_sign_long(number_1));
  set_exp_long(&diff, get_exp_long(number_1));

  if (mantissa_is_greater_long(number_2, number_1)) {
    int temp = get_exp_long(number_1);
    set_exp_long(&number_1, get_exp_long(number_2));
    set_exp_long(&number_1, temp);
    for (int i = 0; i < 6; i++) {
      temp = number_1.bits[i];
      number_1.bits[i] = number_2.bits[i];
      number_2.bits[i] = temp;
    }
    set_sign_long(&diff, !get_sign_long(number_1));
  }
...
void mul(long_dec number_1, long_dec number_2, long_dec *result) {
  long_dec sum = {0};

  for (int i = 0; i < 96; i++) {
    int b2 = get_bit_long(number_2, i);
    if (b2) add(sum, number_1, &sum);
    mantissa_shift_left_long(number_1, &number_1);
  }

  set_sign_long(&sum, abs((get_sign_long(number_1) - get_sign_long(number_2))));
  set_exp_long(&sum, get_exp_long(number_1) + get_exp_long(number_2));
  copy_longdec(sum, result);
}
...
int mantissa_is_zero(long_dec number_1) {
  int flag = 1;
  for (int i = 0; i < 192; i++)
    if (get_bit_long(number_1, i)) {
      flag = 0;
      break;
    }
  return flag;
...
void div_int(long_dec number_1, long_dec number_2, long_dec *result) {
  long_dec quotient = {0}, diff = {0};

  set_sign_long(&diff, get_sign_long(number_2));
  set_exp_long(&diff, get_exp_long(number_2));

  for (int i = 191; i >= 0; i--) {
    mantissa_shift_left_long(diff, &diff);
    mantissa_shift_left_long(quotient, &quotient);
    set_bit_long(&diff, 0, get_bit_long(number_1, i));
    set_bit_long(&quotient, 0, mantissa_is_greater_or_equal(diff, number_2));
    if (mantissa_is_greater_or_equal(diff, number_2))
      sub(diff, number_2, &diff);
  }

  set_sign_long(&quotient,
                abs((get_sign_long(number_1) - get_sign_long(number_2))));
  set_exp_long(&quotient, get_exp_long(number_1) - get_exp_long(number_2));

  if (mantissa_is_zero(number_2)) {
...
void mod(long_dec number_1, long_dec number_2, long_dec *result) {
  long_dec quotient = {0}, diff = {0};
...
void normalize_long(long_dec *number_1, long_dec *number_2) {
  long_dec temp,
      max_dec = {{UINT32_MAX, UINT32_MAX, UINT32_MAX, 0, 0, 0}, 0, 0};
  int n = get_exp_long(*number_2) - get_exp_long(*number_1);
  if (n != 0) {
    long_dec *v = (n > 0 ? number_1 : number_2);
    int pow = get_exp_long(n > 0 ? *number_2 : *number_1);
    copy_longdec(*v, &temp);
    while (get_exp_long(*v) < pow && mantissa_is_less_or_equal(temp, max_dec)) {
      add_exp(temp, v);
      add_exp(temp, &temp);
    }
    n = get_exp_long(*number_2) - get_exp_long(*number_1);
    if (n != 0) {
      long_dec *v = (n > 0 ? number_2 : number_1);
      int pow = get_exp_long(n > 0 ? *number_1 : *number_2);
      copy_longdec(*v, &temp);
...
void div_dec(long_dec number_1, long_dec number_2, long_dec *result) {
  long_dec temp = {0}, diff = {0},
           max_dec = {{UINT32_MAX, UINT32_MAX, UINT32_MAX, 0, 0, 0}, 0, 0};

  while (mantissa_is_less_or_equal(temp, max_dec)) {
    div_int(number_1, number_2, &temp);
    mod(number_1, number_2, &diff);
    if (mantissa_is_zero(diff)) break;
    add_exp(number_1, &number_1);
  }
  copy_longdec(temp, result);
}
...
int mantissa_is_greater(s21_decimal number_1, s21_decimal number_2) {
  int res = 0;
  for (int i = 96; i >= 0; i--)
    if (get_bit(number_1, i) != get_bit(number_2, i)) {
      res = get_bit(number_1, i);
      break;
    }
  return res;
}
...
int getpow(s21_decimal number_1) {
  int result = 0, temp;
  for (int i = 16; i < 24; ++i) {
    temp = s21_get_bit(number_1, i + 96);
    if (temp) {
      result += pow(2, i - 16);
    }
  }
...
void putpow(s21_decimal *number_1, int power) {
  s21_decimal temp;
  nullValue(&temp);
  temp.bits[0] = power;
  for (int i = 0; i < 8; i++) {
    s21_set_bit(number_1, i + 112, s21_get_bit(temp, i));
  }
}
...
int getsign(s21_decimal number_1) { return s21_get_bit(number_1, 127); }
...
int mul_long(long_dec number_1, long_dec number_2, long_dec *result) {
  int bit, resultCode = 0;
  long_dec intermediate;
  nullValueLong(&intermediate);
  nullValueLong(result);
  mulSign_long(number_1, number_2, result);
  for (int i = 0; i < 191 && resultCode == 0; i++) {
    bit = s21_get_bit_long(number_2, i);
    if (bit) {
      if (shiftLeft_long(number_1, i, &intermediate) == 0) {
...
int sumLong(long_dec number_1, long_dec number_2, long_dec *value) {
  int temp, next = 0, result = 0;
  for (int i = 0; i < 191; i++) {
    temp = s21_get_bit_long(number_1, i) + s21_get_bit_long(number_2, i) + next;
    switch (temp) {
      case 3:
...
void put_llintInLongDecimal(long_dec *value, long long number) {
  ...
  s21_set_bit_long(value, 191, (number >> 63 & 1U));
}

int getsign_long(long_dec number_1) { return s21_get_bit_long(number_1, 191); }

void mulSign_long(long_dec number_1, long_dec number_2, long_dec *result) {
  int sign_1 = getsign_long(number_1), sign_2 = getsign_long(number_2);
  if (sign_1 == sign_2) {
...
int s21_greater_long_without_sign(long_dec number_1, long_dec number_2) {
  int result = 2;
  for (int i = 190; i >= 0 && result == 2; i--) {
    if (s21_get_bit_long(number_1, i) > s21_get_bit_long(number_2, i)) {
      result = 0;
    } else if (s21_get_bit_long(number_1, i) < s21_get_bit_long(number_2, i)) {
      result = 1;
    }
  }
  return result;
